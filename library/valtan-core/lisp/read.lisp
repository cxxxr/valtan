...
(defun number-string-p (token)
  "Check if TOKEN could be a number. Returns T if it's a valid number string.
Numbers with decimal points or exponents use base 10.
Integers without decimal use *read-base*."
  (let ((pos (case (aref token 0)
    ...
    ;; First scan: look for decimal point or exponent marker
    (do ((i pos (1+ i)))
        ((>= i (length token)))
      (let ((c (aref token i)))
        (cond ((char= #\. c)
               (setq has-decimal-point t))
              ((member c '(#\e #\s #\f #\d #\l #\E #\S #\F #\D #\L))
               (setq has-exponent t)))))
    ;; If has decimal or exponent, use base 10 rules (original logic)
    (when (or has-decimal-point has-exponent)
      (do ((i pos (1+ i)))
          ((>= i (length token)))
        (let ((c (aref token i)))
          (cond ((char<= #\0 c #\9)
      ...
      (do ((i pos (1+ i)))
        ...
        (let ((c (aref token i)))
          (cond ((char<= #\0 c #\9)
                ...
                (t
                 (return-from number-string-p nil)))))
      (return-from number-string-p found-number-p))
    ;; No decimal or exponent: check using *read-base*
    (do ((i pos (1+ i)))
        ((>= i (length token)))
      (let* ((c (aref token i))
             (digit (digit-char-p c *read-base*)))
        (cond (digit
               (setq found-number-p t))
              ((char= #\/ c)
               ;; Ratio: check rest of token
               (do ((j (1+ i) (1+ j)))
                   ((>= j (length token)))
                 (let ((d (digit-char-p (aref token j) *read-base*)))
                   (unless d
                     (return-from number-string-p nil))))
               (return-from number-string-p found-number-p))
              (t
               ...
               (unless (end-of-string-p)
                 (case (lookahead)
                   ...
                   (#\-
                    (incf pos)
                    t))))
             (digits (&optional (radix 10))
               (let ((digits '()))
                 (do () ((end-of-string-p))
                   (let ((digit (digit-char-p (lookahead) radix)))
                     (unless digit
                       (return))
                     (push digit digits)
                     (next)))
                 digits))
             (digits-to-integer (digits &optional (radix 10))
               (let ((value 0)
                     (i 1))
                 (dolist (digit digits)
                   (incf value (* digit i))
                   (setq i (* i radix)))
                 value))
             (digit* (&optional (radix 10))
               (let ((digits (digits radix)))
                 (digits-to-integer digits radix)))
             (digit+ (&optional (radix 10))
               (let ((digits (digits radix)))
                 (unless (null digits)
                   (digits-to-integer digits radix))))
             (integer-p ()
               ;; Use *read-base* for integers without trailing decimal point
               ;; Use base 10 for integers with trailing decimal point (e.g., "10.")
               (let ((minus (sign))
                     integer
                     has-trailing-decimal)
                 ;; First, check if this has a trailing decimal point
                 ;; by scanning ahead using base 10 digits (since trailing decimal means base 10)
                 (let ((scan-pos pos))
                   (do () ((>= scan-pos len))
                     (let ((c (aref string scan-pos)))
                       (cond ((digit-char-p c 10)  ; Use base 10 for scan
                              (incf scan-pos))
                             ((and (char= c #\.)
                                   (= (1+ scan-pos) len))
                              (setq has-trailing-decimal t)
                              (return))
                             (t (return))))))
                 ;; Now parse with the appropriate base
                 (let ((base (if has-trailing-decimal 10 *read-base*)))
                   (when (and (setq integer (digit+ base))
                              (or (end-of-string-p)
                                  (and (accept #\.)
                                       (end-of-string-p))))
                     (if minus
                         ...
                         integer)))))
             (fraction-p ()
               ;; Use *read-base* for ratios
               (let ((minus (sign))
                     numerator
                     denominator)
                 (when (and (setq numerator (digit+ *read-base*))
                            (accept #\/)
                            (setq denominator (digit+ *read-base*))
                            (end-of-string-p))
                   (if minus
