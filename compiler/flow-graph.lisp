...
(defun create-dominator-table (compiland)
  (let ((d-table (make-hash-table))
        (all-nodes (mapcar #'basic-block-id (compiland-basic-blocks compiland))))
      ...
      (let ((pred (basic-block-pred bb))
        ...
        (if (null pred)
            (setf (gethash self-id d-table)
                  (list self-id))
            (let ((set (gethash (basic-block-id (first pred)) d-table all-nodes)))
              (dolist (p (rest pred))
                (setq set (intersection set (gethash (basic-block-id p) d-table all-nodes))))
              (setf (gethash self-id d-table)
                    (adjoin self-id set))))))
    d-table))
