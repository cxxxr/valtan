...
(defparameter *builtin-function-table*
  (let ((table (make-hash-table)))
    ...
    (setf (gethash '*:%make-symbol table) "lisp.CL_makeSymbol")
    (setf (gethash 'symbol-plist table) "lisp.CL_symbolPlist")
    (setf (gethash '*:put-symbol-plist table) "lisp.CL_setSymbolPlist")
    ...
    (setf (gethash '*:%symbol-name table) "lisp.CL_symbolName")
    (setf (gethash '*:symbol-package-name table) "lisp.CL_symbolPackage")
    (setf (gethash 'system:fset table) "lisp.CL_setSymbolFunction")
    (setf (gethash '*:map-package-symbols table) "lisp.CL_mapPackageSymbols")
    (setf (gethash '*:%put table) "lisp.CL_put")
    (setf (gethash 'packagep table) "lisp.CL_packagep")
    (setf (gethash '*:%package-name table) "lisp.CL_packageName")
    (setf (gethash '*:%package-nicknames table) "lisp.CL_packageNicknames")
    (setf (gethash 'list-all-packages table) "lisp.CL_listAllPackages")
    (setf (gethash '*:intern table) "lisp.CL_intern")
    (setf (gethash '*:find-symbol table) "lisp.CL_findSymbol")
    (setf (gethash '*:make-package table) "lisp.CL_makePackage")
    (setf (gethash 'numberp table) "lisp.CL_numberp")
    (setf (gethash 'integerp table) "lisp.CL_integerp")
    (setf (gethash '*:%add table) "lisp.CL_add")
    (setf (gethash '*:%sub table) "lisp.CL_sub")
    (setf (gethash '*:%negate table) "lisp.CL_negate")
    (setf (gethash '*:%mul table) "lisp.CL_mul")
    (setf (gethash '*:%rem table) "lisp.CL_rem")
    (setf (gethash '*:%= table) "lisp.CL_numberEqual")
    (setf (gethash '*:%/= table) "lisp.CL_numberNotEqual")
    (setf (gethash '*:%> table) "lisp.CL_greaterThan")
    (setf (gethash '*:%< table) "lisp.CL_lessThan")
    (setf (gethash '*:%>= table) "lisp.CL_greaterEqual")
    (setf (gethash '*:%<= table) "lisp.CL_lessEqual")
    (setf (gethash '*:%floor table) "lisp.CL_floor")
    (setf (gethash '*:%logand table) "lisp.CL_logand")
    (setf (gethash '*:apply table) "lisp.CL_apply")
    (setf (gethash 'functionp table) "lisp.CL_functionp")
    (setf (gethash 'consp table) "lisp.CL_consp")
    (setf (gethash 'cons table) "lisp.CL_cons")
    (setf (gethash '*:%car table) "lisp.CL_car")
    (setf (gethash '*:%cdr table) "lisp.CL_cdr")
    (setf (gethash '*:%rplaca table) "lisp.CL_rplaca")
    (setf (gethash '*:%rplacd table) "lisp.CL_rplacd")
    (setf (gethash '*:js-array-to-list table) "lisp.CL_jsArrayToList")
    (setf (gethash '*:list-to-js-array table) "lisp.CL_listToJsArray")
    (setf (gethash 'values table) "lisp.CL_values")
    (setf (gethash '*:multiple-value-call table) "lisp.CL_multipleValueCall")
    (setf (gethash 'system:make-structure table) "lisp.CL_makeStructure")
    (setf (gethash '*:%copy-structure table) "lisp.CL_copyStructure")
    (setf (gethash '*:structure-p table) "lisp.CL_structurep")
    (setf (gethash '*:%structure-name table) "lisp.CL_structureName")
    (setf (gethash '*:%structure-slot-count table) "lisp.CL_structureSlotCount")
    (setf (gethash '*:%structure-ref table) "lisp.CL_structureRef")
    (setf (gethash '*:%structure-set table) "lisp.CL_structureSet")
    (setf (gethash 'eq table) "lisp.CL_eq")
    (setf (gethash '*:error table) "lisp.CL_error")
    (setf (gethash 'characterp table) "lisp.CL_characterp")
    (setf (gethash '*:%code-char table) "lisp.CL_codeChar")
    (setf (gethash '*:%char-code table) "lisp.CL_charCode")
