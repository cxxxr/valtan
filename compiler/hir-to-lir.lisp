    ...
    ((unwind-protect)
     (let ((protected-form (hir-arg1 hir))
       ...
       (let ((result (hir-to-lir protected-form)))
         ...
         result)))
    ((block)
     (let ((block-name-binding (hir-arg1 hir))
           (body (hir-arg2 hir)))
       (setf (binding-init-value block-name-binding) (gen-var))
       (let ((result (emit-lir-forms body)))
         (emit-lir (make-lir 'label (binding-id block-name-binding)))
         result)))
    ((return-from)
     (let ((block-name-binding (hir-arg1 hir))
           (result (hir-to-lir (hir-arg2 hir))))
       (emit-lir (make-lir 'move (binding-init-value block-name-binding) result))
       (emit-lir (make-lir 'jump (binding-id block-name-binding)))
       (binding-init-value block-name-binding)))
    ((tagbody)
     )
    ((go)
     )
