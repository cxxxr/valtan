...
(defun hir-to-lir-1 (hir)
  (ecase (hir-op hir)
    ...
    ((block)
     (let ((name (hir-arg1 hir))
           (body (hir-arg2 hir))
           (block-result (gen-var)))
       (setf (binding-init-value name) block-result)
       (let ((result (emit-lir-forms body)))
         (emit-lir (make-lir 'move block-result result))
...
(defun hir-to-lir (hir)
  (let ((*temp-counter* 0)
        ...
        (*compiland-functions* '())
        (*compiland-body* '()))
    (let ((result (hir-to-lir-1 hir)))
      (emit-lir (make-lir 'return result)))
