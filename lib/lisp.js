...
function checkList(value) {
  checkType(value, (value) => (value instanceof Cons || value === nilValue), "LIST");
}
...
function checkSymbol(value) {
  checkType(value, makeCheckType(Symbol), "SYMBOL");
}
...
cl_package.exportSymbol(
  intern('FUNCALL', cl_package).func = (fn, ...args) => {
    return fn(...args);
  }
);
...
cl_package.exportSymbol(
  intern('SYMBOL-PLIST', cl_package).func = (symbol) => {
    checkArgs(arguments, 1);
    checkSymbol(symbol);
...
intern('STRUCTURE-INDEX', system_package).func = (structure, i) => {
  return values1(structure.index(i));
}
