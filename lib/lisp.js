...
intern('COPY-STRUCTURE', cl_package).func = function (x) {
  checkType(x, makeCheckType(Structure), "STRUCTURE");
  return values1(x.copy());
}

intern('STRUCTURE-P', system_package).func = function (x) {
  return values1(x instanceof Structure);
}

intern('STRUCTURE-NAME', system_package).func = function (structure) {
