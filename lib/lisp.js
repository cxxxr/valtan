...
const S_LISTP = intern('LISTP', cl_package);

function checkList(value) {
  checkType(
    value,
    function(value) {
      if (value === nilValue) return true;
      if (callFunction(S_LISTP, value) !== nilValue) return true;
      return false;
    },
    "LIST");
}
...
const S_LIST_TO_JS_ARRAY = intern('LIST-TO-JS-ARRAY', system_package);

function apply(fn, arg, ...args) {
  fn = ensureFunction(fn);
  if (args.length === 0) {
    checkList(arg);
    const tmp = callFunction(S_LIST_TO_JS_ARRAY, arg);
    return fn(...tmp);
  }
  const vector = [arg];
  ...
  return fn(...vector.concat(callFunction(S_LIST_TO_JS_ARRAY, last)));
}
