...
class LispSymbol {
    ...
    getFunction() {
        if (this.func === null) {
            raise(`The function ${this.name} is undefined.`);
        }
        return this.func;
    }
    ...
    getValue() {
        if (this.value === null) {
            raise(`The variable ${this.name} is unbound`);
        }
        return this.value;
    }
...
export function symbolValue(symbol) {
    if (!symbol.isBound()) raise(`The variable ${symbol.name} is unbound`);
    return symbol.value;
}
...
export function CL_symbolPlist(symbol) {
    checkSymbol(symbol);
    if (symbol.plist === undefined) symbol.plist = nilValue;
...
export function CL_put(symbol, key, value) {
  for (let plist = symbol.plist; plist != nilValue; plist = plist.cdr.cdr) {
    if (plist.car === symbol) {
      plist.cdr.car = value;
