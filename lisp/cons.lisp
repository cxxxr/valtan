...
(defun nintersection (list-1 list-2 &key key (test nil testp) (test-not nil test-not-p))
  (when (and testp test-not-p)
    (error ":TEST and :TEST-NOT were both supplied."))
  (do ((prev nil rest)
       (rest list-1 (cdr rest)))
      ((null rest) list-1)
    (unless (member-aux (car rest) list-2 key test testp test-not test-not-p)
      (if (null prev)
          (pop list-1)
          (rplacd prev (cdr prev))))))
...
(defun nset-difference (list-1 list-2 &key key (test nil testp) (test-not nil test-not-p))
  ...
  (do ((prev nil rest)
       (rest list-1 (cdr rest)))
      ((null rest) list-1)
    (when (member-aux (car rest) list-2 key test testp test-not test-not-p)
      (if (null prev)
          (pop list-1)
          (rplacd prev (cdr prev))))))
...
(defun nset-exclusive-or (list-1 list-2 &rest args &key key test test-not)
  (declare (ignore key test test-not))
  (nconc (apply #'nset-difference list-1 list-2 (cddr args))
         (apply #'nset-difference list-2 list-1 (cddr args))))
