...
(defun gen-get-key-argument (form keyword &optional default)
  `(getf ,form ,keyword ,default))
    ...
    (let ((path arg)
          ...
          (cond ((eq x '&aux)
                ...
                ((eq state :key)
                 ...
                 (cond ((symbolp x)
                        (let ((key (make-keyword x)))
                          (push key keys)
                          (push (list x (gen-get-key-argument path key))
                                *db-bindings*)))
                       ((consp x)
                        (let ((len (length x)))
                          (cond ((<= 1 len 3)
                                 (let ((key (make-keyword (first x))))
                                   ...
                                   (let ((supplied-value (db-gensym))
                                         (default ''#:default))
                                     (push (list supplied-value (gen-get-key-argument path key default))
                                           *db-bindings*)
                                     (when (= len 3)
                ...
                ((listp x)
                 ...
                 (let ((car-var (db-gensym "TMP")))
                   (push (list car-var `(car ,path)) *db-bindings*)
      ...
      (when (and check-key-placeholder
        ...
        (setf (second check-key-placeholder)
              (let ((rest (gensym)))
                `(do ((,rest ,(second check-key-placeholder) (cddr ,rest)))
                     ((null ,rest))
                   (unless (or ,@(mapcar (lambda (key) `(eq ,key (car ,rest))) keys))
                     (error "Unknown &key argument: ~S" (car ,rest))))))))))
